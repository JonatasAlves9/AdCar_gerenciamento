...
function carEdit({ history, match }) {
  ...
  async function showCar() {
    try {
      ...
      setUrl_master(data.car.url_master)
      setDescription(data.car.description)
      setGearbox(data.car.gearbox)
      setImagem(data.url)
    } catch (error) {
      console.log(error)
  ...
  const imagesForm = imagem.map(image =>
    <Form.Group key={image.idCar} controlId="formGroupEmail">
      ...
      <div className={styles.pag}>
        <div className={styles.cardLeft}>
          ...
          <Form>
            ...
            <Button className={styles.buttonDelete} variant="danger" onClick={() => handleDeleteCar()}>
              Remover
            </Button>
          </Form>
        </div>
